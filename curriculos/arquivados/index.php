...
function AcoesCellRenderer(params) {
  ...
  linkView.href = './ver/' + params.data.nome.toLowerCase().replace(/\s+/g, '-') + '/?id=' + params.data.id;
...
const columnDefs = [
  ...
  { field: 'nome', filter: true, width: 300, headerName: 'Nome' },
  { field: 'email', filter: true, headerName: 'E-mail' },
  { field: 'telefone', filter: true, headerName: 'Telefone' },
  { field: 'endereco', filter: true, headerName: 'Endereço' },
  {
    field: 'acoes',
    filter: false,
    headerName: 'Ações',
...
const rowData = [
  ...
  while($dado = $con->fetch_array()) { ?> 

    {
      ...
      endereco: "<?php echo $dado['endereco'] ?>"
    },
